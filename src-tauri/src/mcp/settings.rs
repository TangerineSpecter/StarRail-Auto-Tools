...
pub fn catalog_tools() -> Vec<McpToolInfo> {
    vec![
        ...
        McpToolInfo {
            ...
            description: "必须传入 start_game_data_capture 返回的任务 ID，查询当前提示及已采集的数据数量。未拿到任务 ID 时不得调用。建议每 2 到 3 秒调用一次直到 terminal=true；已配置 SFTP 时 completed 表示已完成上传。".to_owned(),
            destructive: false,
        },
    ]
}
