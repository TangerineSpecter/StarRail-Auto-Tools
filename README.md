...
[![Version](https://img.shields.io/badge/version-1.0.9-4f7fc4?style=flat-square)](https://github.com/TangerineSpecter/StarRail-Auto-Tools/releases)
...
- 🗺️ **图鉴与培养规划**：内置遗器、位面饰品、角色与光锥参考数据，支持角色毕业目标、完整站街属性门槛与六部位全局配装 Top 5。
...
- Windows 游戏数据直读基于系统 Packet Monitor 与 `reliquary v23.0.0`（适配游戏 4.5），无需安装 Npcap。
...
数据录入页的“监听日志”会显示抓包启动、候选 UDP 包、游戏握手、命令解析及入库结果；状态快照中的 `logs` 最多保留 80 条，不包含原始网络包。长时间没有收到包时，检查是否已在监听就绪后从登录界面重新进入游戏；有握手却没有命令或提示密钥解析失败时，留意内置 `reliquary v23.0.0` 对应的游戏版本。可在“关于”导出包含后端监听日志的分析报告，供进一步排查。
